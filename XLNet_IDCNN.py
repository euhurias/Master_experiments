...
class XLNetIDCNN(nn.Module):
    def __init__(self, base_model, num_labels, dropout_rate=0.1, filters=64):
        super().__init__()
        self.xlnet = base_model
        hidden_size = self.xlnet.config.hidden_size
        
        self.dilated_convs = nn.ModuleList([
            nn.Conv1d(hidden_size, filters, 3, padding=2, dilation=2),
            nn.Conv1d(filters, filters, 3, padding=4, dilation=4),
            nn.Conv1d(filters, filters, 3, padding=8, dilation=8),
            nn.Conv1d(filters, filters, 3, padding=16, dilation=16)
        ])
        
        self.dropout = nn.Dropout(dropout_rate)
        self.classifier = nn.Linear(filters, num_labels)
...
def train_fold(fold, train_loader, val_loader, label_map, args):
    ...
    model = XLNetIDCNN(base_model.xlnet, len(label_map))
    ...
    for epoch in range(args.epochs):
        ...
        if results['f1'] > best_f1:
            ...
            torch.save(model.state_dict(), 
                      os.path.join(args.output_dir, f'fold_xlnet_idcnn_{fold}_best.pt'))
        else:
            patience += 1
...
def main():
    ...
    logging.basicConfig(
        ...
        format="%(message)s",
        handlers=[
            logging.FileHandler(os.path.join(args.output_dir, 'resultados_xlnet_idcnn.txt')),
    ...
    logging.info("\n=== Resultados Finais ===")
    logging.info(f"Modelo: XLNet + IDCNN")
