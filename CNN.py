...
class CNN_NER(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_filters, filter_sizes, num_labels, dropout=0.1):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embedding_dim, padding_idx=0)
        self.convs = nn.ModuleList([
            nn.Conv1d(embedding_dim, num_filters, kernel_size=fs, padding=(fs-1)//2)
            for fs in filter_sizes
        ])
        self.dropout = nn.Dropout(dropout)
        # Classificador por token - saída para cada posição na sequência
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        # Reorganizar para CNN: [batch_size, embedding_dim, seq_len]
        embeddings = embeddings.transpose(1, 2)
        
        # Aplicar convoluções - manter dimensão temporal
        conv_outputs = []
        for conv in self.convs:
            conv_out = F.relu(conv(embeddings))  # [batch_size, num_filters, seq_len]
        ...
        # Reorganizar para: [batch_size, seq_len, num_filters * len(filter_sizes)]
        concatenated = concatenated.transpose(1, 2)
        
        # Classificação por token
        logits = self.classifier(concatenated)  # [batch_size, seq_len, num_labels]
        
        if labels is not None:
            loss_fct = nn.CrossEntropyLoss()
            # Aplicar máscara para ignorar tokens padding
            active_loss = attention_mask.view(-1) == 1
            active_logits = logits.view(-1, logits.size(-1))
            active_labels = labels.view(-1)
            
            # Apenas calcular loss para tokens não-padding
...
def convert_examples_to_features(examples, tokenizer, processor):
    ...
    for ex in examples:
        ...
        for word, label in zip(ex.words, ex.labels):
            ...
            predict_mask.extend([1] + [0]*(len(word_tokens)-1))  # Apenas primeiro token é predito
        ...
        predict_mask.append(0)  # [SEP] não é predito
...
def evaluate(model, dataloader, processor):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            predict_mask_batch = predict_mask.cpu().numpy()
            
            for i in range(len(preds)):
                current_true = []
                current_pred = []
                for j in range(len(predict_mask_batch[i])):
                    if predict_mask_batch[i][j]:  # Apenas tokens marcados para predição
    ...
    return {
        ...
        'precision': precision,
        'recall': recall,
        'report': classification_report(true_labels, pred_labels, zero_division=0) if true_labels else "No predictions"
    }
...
def train_fold(fold, train_loader, val_loader, processor, args, tokenizer):
    ...
    best_f1 = -1
    patience_counter = 0
    best_model_path = os.path.join(args.output_dir, f"fold_CNN_{fold}_best.pt")
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(args.output_dir, 'resultados_cnn.txt')),
            logging.StreamHandler(sys.stdout)
        ]
    )
    
    try:
        logging.info("🚀 INICIANDO TREINAMENTO CNN")
        ...
        logging.info(f"🔤 Tamanho do vocabulário: {tokenizer.vocab_size}")
        ...
        with open(os.path.join(args.output_dir, 'metricas_finais_cnn.txt'), 'w') as f:
