...
def train_fold(fold, train_loader, val_loader, processor, tokenizer, args):
    ...
    best_f1 = -1
    patience_counter = 0
    best_model_path = os.path.join(args.output_dir, f"fold_{fold}_best_electra.pt")
...
def main():
    ...
    logging.info("=" * 70)
    logging.info("🚀 INICIANDO TREINAMENTO - ELECTRA LINEAR")
    ...
    try:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(examples)):
            ...
            with open(os.path.join(args.output_dir, f'fold_{fold+1}_report.txt'), 'w') as f:
                f.write(f"FOLD {fold+1} - ELECTRA LINEAR\n")
                f.write("="*50 + "\n")
        ...
        with open(os.path.join(args.output_dir, 'final_results.txt'), 'w') as f:
            f.write("RESULTADOS FINAIS - ELECTRA LINEAR\n")
            f.write("="*60 + "\n")
            f.write(f"F1 Médio: {final_f1:.4f} (±{final_std:.4f})\n\n")
            for i, metrics in enumerate(all_metrics, 1):
                f.write(f"Fold {i}: F1={metrics['f1']:.4f}\n")
        
        logging.info(f"\n✅ Treinamento concluído! 📁 {args.output_dir}")
