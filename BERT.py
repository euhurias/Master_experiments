...
def parse_args():
    ...
    parser.add_argument('--dropout', type=float, default=0.1, help='Ignorado (modelo pré-definido)')
...
def convert_examples_to_features(examples, tokenizer, processor, max_seq_length):
    ...
    for ex_idx, example in enumerate(examples):
        ...
        for word, label in zip(example.words, example.labels):
            ...
            label_ids.extend([main_label] + [-100] * (len(word_tokens) - 1))
        ...
        for batch in tqdm(dataloader, desc="Avaliando", leave=False):
            ...
            for i in range(len(preds)):
                ...
                for j in range(len(predict_mask[i])):
                    if predict_mask[i][j] and labels_np[i][j] != -100:
...
def train_fold(fold, train_loader, val_loader, processor, args, class_weights=None):
    ...
    # Se usar pesos de classe, precisamos modificar a loss do modelo?
    # O BertForTokenClassification usa CrossEntropyLoss internamente e permite passar weights.
    # Podemos substituir a loss function.
    if args.use_class_weights and class_weights is not None:
        model.config.problem_type = "single_label_classification"
        # Acessa o classificador e redefine a loss
        # A maneira correta é reatribuir a loss function do modelo.
        # O modelo armazena a loss function internamente; podemos substituí-la.
        # Vamos fazer isso após a criação.
        model.num_labels = len(processor.labels)
        # A loss function padrão é CrossEntropyLoss, podemos substituir:
        model.loss_fct = nn.CrossEntropyLoss(weight=class_weights)
        # Nota: isso não é oficialmente suportado, mas funciona.
        # Para evitar problemas, poderíamos também usar model.config.problem_type e confiar que o modelo usará a loss padrão.
        # Mas como não temos controle total, a abordagem abaixo é mais segura:
        # Vamos definir model.loss_fct manualmente.
        # O forward do modelo chama self.loss_fct se existir, senão cria uma padrão.
        # Portanto, definir model.loss_fct deve funcionar.
        model.loss_fct = nn.CrossEntropyLoss(weight=class_weights)
    ...
    best_f1 = -1
    patience_counter = 0
    best_model_path = os.path.join(args.output_dir, f"bert_fold_{fold}_best.pt")
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(args.output_dir, 'bert_training.log')),
            logging.StreamHandler(sys.stdout)
        ]
    )

    # Salva argumentos
    with open(os.path.join(args.output_dir, 'bert_args.json'), 'w') as f:
        json.dump(vars(args), f, indent=2)

    logging.info("=" * 70)
    logging.info("🚀 INICIANDO TREINAMENTO - BERT (Token Classification)")
    ...
    try:
        ...
        # Calcula pesos de classe se solicitado
        class_weights = None
        if args.use_class_weights:
            all_labels = [lb for ex in examples for lb in ex.labels]
            unique_labels = sorted(list(set(all_labels)))
            # Garante que a ordem dos pesos corresponda ao índice do label_map
            label_to_idx = processor.label_map
            weights = []
            for lbl in processor.labels:  # percorre na ordem do mapeamento
                if lbl in unique_labels and lbl not in ['[PAD]', '[CLS]', '[SEP]', 'X']:
                    count = all_labels.count(lbl)
                    if count > 0:
                        weight = len(all_labels) / (len(unique_labels) * count)
                    else:
                        weight = 1.0
                else:
                    weight = 1.0
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(examples)):
            ...
            with open(os.path.join(args.output_dir, f'bert_fold_{fold+1}_report.txt'), 'w') as f:
                f.write(f"FOLD {fold+1} - BERT (Token Classification)\n")
                f.write("="*50 + "\n")
        ...
        logging.info(f"\n{'='*70}")
        logging.info("🎯 RESULTADOS FINAIS - BERT (Token Classification)")
        logging.info(f"{'='*70}")
        ...
        with open(os.path.join(args.output_dir, 'bert_final_metrics.json'), 'w') as f:
            json.dump(final_metrics, f, indent=2, ensure_ascii=False)

        with open(os.path.join(args.output_dir, 'bert_final_results.txt'), 'w') as f:
            f.write("RESULTADOS FINAIS - BERT (Token Classification)\n")
            f.write("="*60 + "\n")
