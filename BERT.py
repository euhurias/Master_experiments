...
def train_fold(fold, train_loader, val_loader, processor, args):
    ...
    best_f1 = 0
    patience_counter = 0
    best_model_path = os.path.join(args.output_dir, f"fold_BERT_{fold}_best.pt")
...
def main():
    ...
    logging.basicConfig(
        ...
        format="%(message)s",
        handlers=[
            logging.FileHandler(os.path.join(args.output_dir, 'resultados_bert.txt')),
