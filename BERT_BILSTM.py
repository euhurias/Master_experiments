...
def parse_args():
    ...
    parser.add_argument('--use_class_weights', action='store_true',
                        help='Usar pesos de classe na loss (padrão: False)')
...
def convert_examples_to_features(examples, tokenizer, processor, max_seq_length):
    ...
    for ex_idx, example in enumerate(examples):
        ...
        for word, label in zip(example.words, example.labels):
            ...
            # Subpalavras adicionais recebem -100 (ignoradas na loss) e não são previstas
...
class BERTBiLSTM(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels=None, predict_mask=None, class_weights=None):
        ...
        if labels is not None:
            # Aplica máscaras: ignorar tokens -100 e onde predict_mask=0
...
def train_fold(fold, train_loader, val_loader, processor, args, class_weights=None):
    ...
    best_f1 = -1
    patience_counter = 0
    best_model_path = os.path.join(args.output_dir, f"bert_bilstm_fold_{fold}_best.pt")
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(args.output_dir, 'bert_bilstm_training.log')),
            logging.StreamHandler(sys.stdout)
        ]
    )

    # Salva argumentos
    with open(os.path.join(args.output_dir, 'bert_bilstm_args.json'), 'w') as f:
        json.dump(vars(args), f, indent=2)

    logging.info("=" * 70)
    logging.info("🚀 INICIANDO TREINAMENTO - BERT BiLSTM (sem CRF)")
    ...
    try:
        ...
        if args.use_class_weights:
            ...
            for lbl in processor.labels:  # percorre na ordem do mapeamento
                if lbl in unique_labels:
                    # compute_class_weight espera classes ordenadas e y
                    # Vamos calcular manualmente para simplificar
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(examples)):
            ...
            with open(os.path.join(args.output_dir, f'bert_bilstm_fold_{fold+1}_report.txt'), 'w') as f:
                f.write(f"FOLD {fold+1} - BERT BiLSTM (sem CRF)\n")
                f.write("="*50 + "\n")
        ...
        logging.info(f"\n{'='*70}")
        logging.info("🎯 RESULTADOS FINAIS - BERT BiLSTM (sem CRF)")
        logging.info(f"{'='*70}")
        ...
        with open(os.path.join(args.output_dir, 'bert_bilstm_final_metrics.json'), 'w') as f:
            json.dump(final_metrics, f, indent=2, ensure_ascii=False)

        with open(os.path.join(args.output_dir, 'bert_bilstm_final_results.txt'), 'w') as f:
            f.write("RESULTADOS FINAIS - BERT BiLSTM (sem CRF)\n")
            f.write("="*60 + "\n")
