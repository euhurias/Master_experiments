...
def train_fold(fold, train_loader, val_loader, processor, args, class_weights=None):
    ...
    model = BertIDCNN(
        ...
        num_blocks=args.cnn_num_blocks,
    ...
    best_f1 = -1
    patience_counter = 0
    best_model_path = os.path.join(args.output_dir, f"bert_idcnn_fold_{fold}_best.pt")
    ...
    logging.info(f"  CNN kernel_size: {args.cnn_kernel_size}")
    logging.info(f"  CNN num_blocks: {args.cnn_num_blocks}")
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(args.output_dir, 'bert_idcnn_training.log')),
            logging.StreamHandler(sys.stdout)
        ]
    )

    # Salva argumentos
    with open(os.path.join(args.output_dir, 'bert_idcnn_args.json'), 'w') as f:
    ...
    try:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(examples)):
            ...
            with open(os.path.join(args.output_dir, f'bert_idcnn_fold_{fold+1}_report.txt'), 'w') as f:
                f.write(f"FOLD {fold+1} - BERT IDCNN\n")
                f.write("="*50 + "\n")
        ...
        logging.info(f"\n{'='*70}")
        logging.info("🎯 RESULTADOS FINAIS - BERT IDCNN")
        logging.info(f"{'='*70}")
        ...
        with open(os.path.join(args.output_dir, 'bert_idcnn_final_metrics.json'), 'w') as f:
            json.dump(final_metrics, f, indent=2, ensure_ascii=False)

        with open(os.path.join(args.output_dir, 'bert_idcnn_final_results.txt'), 'w') as f:
            f.write("RESULTADOS FINAIS - BERT IDCNN\n")
            f.write("="*60 + "\n")
