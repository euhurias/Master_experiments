...
class XLNetBiLSTMCNN(nn.Module):
    def __init__(self, base_model, num_labels, dropout_rate=0.1, lstm_hidden=256, cnn_filters=128):
        super().__init__()
        ...
        self.conv = nn.Conv1d(lstm_hidden * 2, cnn_filters, 3, padding=1)
...
def train_fold(fold, train_loader, val_loader, label_map, args):
    ...
    model = XLNetBiLSTMCNN(base_model.xlnet, len(label_map))
    ...
    for epoch in range(args.epochs):
        ...
        if results['f1'] > best_f1:
            ...
            torch.save(model.state_dict(), 
                      os.path.join(args.output_dir, f'fold_xlnet_bilstm_cnn_{fold}_best.pt'))
        else:
            patience += 1
...
def main():
    ...
    logging.basicConfig(
        ...
        format="%(message)s",
        handlers=[
            logging.FileHandler(os.path.join(args.output_dir, 'resultados_xlnet_bilstm_cnn.txt')),
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(examples)):
        ...
        print(f"Modelo: XLNet + BiLSTM + CNN")
    ...
    logging.info("\n=== Resultados Finais ===")
    logging.info(f"Modelo: XLNet + BiLSTM + CNN")
