...
class DataProcessor:
    ...
    def _create_label_maps(self, labels):
        # Para CRF, não precisamos de tokens especiais, apenas as labels reais.
    ...
    with torch.no_grad():
        for sentences, labels, mask in tqdm(dataloader, desc="Avaliando", leave=False):
            ...
            for i, seq_preds in enumerate(preds_list):
                ...
                preds[i, :seq_len] = torch.tensor(seq_preds[:seq_len])
            ...
            for i in range(len(preds_np)):
                ...
                for j in range(mask_np.shape[1]):
                    if mask_np[i][j] and labels_np[i][j] != -1:
                        ...
                        pred_label = processor.reverse_label_map[preds_np[i][j]]
...
def train_fold(fold, train_loader, val_loader, processor, args, vocab_size):
    ...
    best_f1 = -1
    patience_counter = 0
    best_model_path = os.path.join(args.output_dir, f"bilstm_crf_fold_{fold}_best.pt")
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(args.output_dir, 'bilstm_crf_training.log')),
            logging.StreamHandler(sys.stdout)
        ]
    )

    # Salva argumentos
    with open(os.path.join(args.output_dir, 'bilstm_crf_args.json'), 'w') as f:
        json.dump(vars(args), f, indent=2)

    logging.info("=" * 70)
    logging.info("🚀 INICIANDO TREINAMENTO - BiLSTM-CRF (from scratch)")
    ...
    try:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(examples)):
            ...
            with open(os.path.join(args.output_dir, f'bilstm_crf_fold_{fold+1}_report.txt'), 'w') as f:
                f.write(f"FOLD {fold+1} - BiLSTM-CRF\n")
                f.write("="*50 + "\n")
        ...
        logging.info(f"\n{'='*70}")
        logging.info("🎯 RESULTADOS FINAIS - BiLSTM-CRF")
        logging.info(f"{'='*70}")
        ...
        with open(os.path.join(args.output_dir, 'bilstm_crf_final_metrics.json'), 'w') as f:
            json.dump(final_metrics, f, indent=2, ensure_ascii=False)

        with open(os.path.join(args.output_dir, 'bilstm_crf_final_results.txt'), 'w') as f:
            f.write("RESULTADOS FINAIS - BiLSTM-CRF\n")
            f.write("="*60 + "\n")
