...
def evaluate(model, dataloader, processor):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Avaliando", leave=False):
            ...
            preds_list = model.decode(logits, attention_mask)
            ...
            attention_mask_np = attention_mask[:, 1:-1].cpu().numpy()

            for i in range(len(preds)):
                current_true = []
                current_pred = []
                for j in range(attention_mask_np.shape[1]):
                    if attention_mask_np[i][j] and labels_np[i][j] != -100:
...
def train_fold(fold, train_loader, val_loader, processor, args):
        ...
        for i in range(len(labels)):
            for j in range(len(labels[i])):
                if mask[i][j] and labels[i][j] != -100:
                    # Desconsiderar [CLS] e [SEP] no índice
                    if j > 0 and j < mask.shape[1] - 1:
    ...
    other_params = [p for n, p in model.named_parameters() if not n.startswith('bilstm')]
    ...
    best_f1 = -1
    patience_counter = 0
    best_model_path = os.path.join(args.output_dir, f"xlnet_bilstm_crf_fold_{fold}_best.pt")
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(args.output_dir, 'xlnet_bilstm_crf_training.log')),
            logging.StreamHandler(sys.stdout)
        ]
    )

    # Salva argumentos
    with open(os.path.join(args.output_dir, 'xlnet_bilstm_crf_args.json'), 'w') as f:
        json.dump(vars(args), f, indent=2)

    logging.info("=" * 70)
    logging.info("🚀 INICIANDO TREINAMENTO - XLNet + BiLSTM + CRF")
    ...
    try:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(examples)):
            ...
            with open(os.path.join(args.output_dir, f'xlnet_bilstm_crf_fold_{fold+1}_report.txt'), 'w') as f:
                f.write(f"FOLD {fold+1} - XLNet + BiLSTM + CRF\n")
                f.write("="*50 + "\n")
        ...
        logging.info(f"\n{'='*70}")
        logging.info("🎯 RESULTADOS FINAIS - XLNet + BiLSTM + CRF")
        logging.info(f"{'='*70}")
        ...
        with open(os.path.join(args.output_dir, 'xlnet_bilstm_crf_final_metrics.json'), 'w') as f:
            json.dump(final_metrics, f, indent=2, ensure_ascii=False)

        with open(os.path.join(args.output_dir, 'xlnet_bilstm_crf_final_results.txt'), 'w') as f:
            f.write("RESULTADOS FINAIS - XLNet + BiLSTM + CRF\n")
            f.write("="*60 + "\n")
