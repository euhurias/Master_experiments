...
class Config:
    llama_model_name = "meta-llama/Llama-2-7b-hf"  # Ajuste conforme seu ambiente
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    seed = 42

class LlamaLoRA(nn.Module):
    def __init__(self, model_name, num_labels, lora_r=16, lora_alpha=32):
        super().__init__()
        ...
        # Classifier para NER - adaptado para LLaMA
        self.classifier = nn.Linear(self.llama.config.hidden_size, num_labels)
        
        # Log LoRA parameters
        trainable_params = sum(p.numel() for p in self.llama.parameters() if p.requires_grad)
...
def train_fold(fold, train_loader, val_loader, processor, args, tokenizer):
    try:
        ...
        best_f1 = -1
        patience_counter = 0
        best_model_path = os.path.join(args.output_dir, f"fold_LLaMA_LoRA_{fold}_best.pt")
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(args.output_dir, 'resultados_llama_lora.txt')),
            logging.StreamHandler(sys.stdout)
        ]
    )
    
    try:
        logging.info("🚀 INICIANDO TREINAMENTO LLaMA + LoRA")
        ...
        with open(os.path.join(args.output_dir, 'metricas_finais_llama_lora.txt'), 'w') as f:
