...
try:
    ...
    CRF_AVAILABLE = False
    print("⚠️  Instale CRF: pip install pytorch-crf")
...
class ElectraCRF(nn.Module):
    def __init__(self, model_name, num_labels, dropout=0.1):
        ...
        if not CRF_AVAILABLE:
            raise ImportError("CRF não disponível")
        ...
        for batch in tqdm(dataloader, desc="Avaliando", leave=False):
            ...
            for i in range(len(preds)):
                ...
                for j in range(len(preds[i])):
                    if j < len(predict_mask[i]) and predict_mask[i][j]:
...
def train_fold(fold, train_loader, val_loader, processor, tokenizer, args):
    model = ElectraCRF(args.model_name, len(processor.labels), args.dropout).to(Config.device)
    ...
    best_f1 = -1
    patience_counter = 0
    best_model_path = os.path.join(args.output_dir, f"fold_electra_crf_{fold}_best.pt")
    ...
    for epoch in range(args.epochs):
        ...
        progress_bar = tqdm(train_loader, desc=f"Fold_electra_crf_{fold} - Época {epoch+1}", leave=False)
...
def main():
    ...
    if not CRF_AVAILABLE:
        print("❌ CRF não está disponível. Instale com: pip install pytorch-crf")
        sys.exit(1)
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(args.output_dir, 'training_electra_crf.log')),
            logging.StreamHandler(sys.stdout)
        ]
    )
    
    with open(os.path.join(args.output_dir, 'args_electra_crf.json'), 'w') as f:
        json.dump(vars(args), f, indent=2)
    
    logging.info("=" * 70)
    logging.info("🚀 INICIANDO TREINAMENTO - ELECTRA CRF")
    ...
    try:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(examples)):
            ...
            with open(os.path.join(args.output_dir, f'fold_electra_crf_{fold+1}_report.txt'), 'w') as f:
                f.write(f"FOLD {fold+1} - ELECTRA CRF\n")
                f.write("="*50 + "\n")
        ...
        logging.info(f"\n{'='*70}")
        logging.info("🎯 RESULTADOS FINAIS - ELECTRA CRF")
        logging.info(f"{'='*70}")
        ...
        with open(os.path.join(args.output_dir, 'final_metrics_electra_crf.json'), 'w') as f:
            json.dump(final_metrics, f, indent=2, ensure_ascii=False)
        
        with open(os.path.join(args.output_dir, 'final_electra_crf_results.txt'), 'w') as f:
            f.write("RESULTADOS FINAIS - ELECTRA CRF\n")
            f.write("="*60 + "\n")
