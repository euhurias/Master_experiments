...
def train_fold(fold, train_loader, val_loader, processor, args):
    ...
    best_f1 = -1
    patience_counter = 0
    best_model_path = os.path.join(args.output_dir, f"fold_BERT_CNN_{fold}_best.pt")
    
    logging.info(f"🔧 Configuração BERT+CNN:")
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(args.output_dir, 'resultados_bert_cnn.txt')),
    ...
    try:
        ...
        with open(os.path.join(args.output_dir, 'metricas_finais_bert_cnn.txt'), 'w') as f:
