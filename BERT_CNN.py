...
def parse_args():
    ...
    parser.add_argument('--cnn_num_filters', type=int, default=100)
    parser.add_argument('--cnn_filter_sizes', type=str, default='3,5,7',
...
class BertCNN(nn.Module):
    def __init__(self, model_name, num_filters, filter_sizes, num_labels, dropout=0.1):
        super().__init__()
    ...
    def forward(self, input_ids, attention_mask, labels=None, class_weights=None):
        ...
        # Transpor para (batch, embedding_dim, seq_len) para convoluções
        sequence_output = sequence_output.transpose(1, 2)

        conv_outputs = []
        for conv in self.convs:
            conv_out = F.relu(conv(sequence_output))
            conv_out = conv_out.transpose(1, 2)  # volta para (batch, seq_len, num_filters)
            conv_outputs.append(conv_out)

        concatenated = torch.cat(conv_outputs, dim=-1)  # (batch, seq_len, num_filters * len(filter_sizes))
        ...
        for batch in tqdm(dataloader, desc="Avaliando", leave=False):
            ...
            for i in range(len(preds)):
                ...
                for j in range(len(masks[i])):
                    if masks[i][j] and labels_np[i][j] != -100:
...
def train_fold(fold, train_loader, val_loader, processor, args, class_weights=None):
    ...
    # Converter filter_sizes de string para lista de inteiros
    ...
    best_f1 = -1
    patience_counter = 0
    best_model_path = os.path.join(args.output_dir, f"bert_cnn_fold_{fold}_best.pt")

    logging.info(f"🔧 Configuração Fold {fold}:")
    logging.info(f"  Labels: {len(processor.labels)}")
    logging.info(f"  Batch size: {args.batch_size}")
    logging.info(f"  CNN num_filters: {args.cnn_num_filters}")
    logging.info(f"  CNN filter_sizes: {filter_sizes}")
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(args.output_dir, 'bert_cnn_training.log')),
            logging.StreamHandler(sys.stdout)
        ]
    )

    # Salva argumentos
    with open(os.path.join(args.output_dir, 'bert_cnn_args.json'), 'w') as f:
        json.dump(vars(args), f, indent=2)

    logging.info("=" * 70)
    logging.info("🚀 INICIANDO TREINAMENTO - BERT CNN")
    ...
    try:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(examples)):
            ...
            with open(os.path.join(args.output_dir, f'bert_cnn_fold_{fold+1}_report.txt'), 'w') as f:
                f.write(f"FOLD {fold+1} - BERT CNN\n")
                f.write("="*50 + "\n")
        ...
        logging.info(f"\n{'='*70}")
        logging.info("🎯 RESULTADOS FINAIS - BERT CNN")
        logging.info(f"{'='*70}")
        ...
        with open(os.path.join(args.output_dir, 'bert_cnn_final_metrics.json'), 'w') as f:
            json.dump(final_metrics, f, indent=2, ensure_ascii=False)

        with open(os.path.join(args.output_dir, 'bert_cnn_final_results.txt'), 'w') as f:
            f.write("RESULTADOS FINAIS - BERT CNN\n")
            f.write("="*60 + "\n")
