...
try:
    ...
    print("Aviso: PEFT não está instalado. Instale com: pip install peft")
...
def train_fold(fold, train_loader, val_loader, label_map, args):
    if not PEFT_AVAILABLE:
        raise ImportError("PEFT não está instalado. Instale com: pip install peft")
    ...
    print(f"LoRA aplicado: {model.print_trainable_parameters()}")
    ...
    for epoch in range(args.epochs):
        ...
        if results['f1'] > best_f1:
            ...
            torch.save(model.state_dict(), 
                      os.path.join(args.output_dir, f'fold_xlnet_lora_{fold}_best.pt'))
        else:
            patience += 1
...
def main():
    ...
    if not PEFT_AVAILABLE:
        print("Erro: PEFT não está instalado. Instale com: pip install peft")
        sys.exit(1)
    ...
    logging.basicConfig(
        ...
        format="%(message)s",
        handlers=[
            logging.FileHandler(os.path.join(args.output_dir, 'resultados_xlnet_lora.txt')),
    ...
    logging.info("\n=== Resultados Finais ===")
    logging.info(f"Modelo: XLNet + LoRA")
