...
def train_fold(fold, train_loader, val_loader, processor, args):
    model = ElmoLSTMCRF(num_labels=len(processor.labels)).to(Config.device)
    optimizer = AdamW(model.parameters(), lr=args.learning_rate)
    
    best_f1 = -1
    patience_counter = 0
    best_model_path = os.path.join(args.output_dir, f"fold_ELMo_LSTM_CRF_{fold}_best.pt")
    
    logging.info(f"🔧 Configuração ELMo+LSTM+CRF:")
    ...
    return evaluate_crf(model, val_loader, processor)
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(args.output_dir, 'resultados_elmo_lstm_crf.txt')),
            logging.StreamHandler(sys.stdout)
        ]
    )
    
    try:
        logging.info("🚀 INICIANDO TREINAMENTO ELMo+LSTM+CRF")
        ...
        with open(os.path.join(args.output_dir, 'metricas_finais_elmo_lstm_crf.txt'), 'w') as f:
